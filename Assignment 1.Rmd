...
d_svm_test <- data.frame(test$Value, predictYsvm)
 colnames(d_svm_test) <- c("obs", "pred")
   ...
   p2<-ggplot(d_svm_test, mapping = aes(y = residuals, x = 0, label = label)) +
      geom_boxplot(orientation = "vertical") +
...
-90% population will be vaccinated at the end of November.
...
For Polynomial Regression we will find the optimal coefficient for the  training dataset and check how the model performs on  the testing dataset. Polynomial regression works on any size of the dataset and works well on non-liner problems. The main challenge is choosing the right polynomial degree for good bias-variance tradeoff. 
...
SVM is showing very good model performance and I think we have overfitting as the predicted result doesn't look realistic. Also I tried to use a radial kernel in SVM to reduce the overfitting but it reduced the r-squared to almost 0.15 and a lot of residual outliers have been detected for the training and testing dataset.  

I think the Polynomial Regression performs better than SVM and the herd immunity can be achieved in January when the 90% of NZ population can be potentially vaccinated.
