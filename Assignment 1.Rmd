...
Box plot allows to identify outliers for a numeric data. There are some outliers detected but because this is a cumulative data the latest observations are treated as outliers.
...
The first 80% of data is assigned to a training dataset and the last 20% is assigned to a testing dataset.
We will traing our method using training dataset and find the optimal hyper parameters for our model. The model with the optimal hyperparameters will be applied to the training dataset. 
...
employ.data
```
Above table is showing the SVM performs too good and there is a high risk of overfitting. 
...
As per above plot the predicted values are higher then actual at the end which means the projection could potentially overestimate the number of vaccinations.
...
employ.data
```
Polynomial Regression model is showing a good result on a testing dataset. Also we can control the function degree adding or removing variables. The optimal number is 2 as it gives us R-squared around 0.62, so our model is not overfitting.
We can plot the predicted and testing values to see how our model behaves. 
...
Vaccin_DHB dataframe contains 28 observations and 9 variables. After the data is loaded there are three data types detected: character, numeric and integer. I have updated the `DHB of residence`, `Age group`, `Ethnic group`, `Gender` to a factor as these variables have a low cardinality.
...
Low outliers are showing for the age group 55-90+. 
...
```
### Total Vaccinations
Data in this section is as at 11:59pm 16 September 2021 and is updated each week day (Monday to Friday).
...
library(scales)
#df102 = Vaccin_DHB%>%
#  summarise(Sum_first = sum(`First dose administered`), Sum_second = sum(`Second dose administered`), pop=sum(`Population`) )
...
ggplot(Vaccin_admin, aes(x = "", y =Vaccin, fill =`Label2`)) +
  ...
  ggtitle("% of doses administered around NZ")+
  coord_flip()+
  xlab("First/Second dose") +
  ylab("% of doses administered")
```
...
ggplot(df100, aes(x = New_Var, y =Sum/pop, fill =`DHB of residence`)) +
  geom_bar(stat = "identity", color = "black",position = position_dodge(), width = 0.7)+
  theme(axis.text.x = element_text(face = "bold",angle = 90),
  legend.title = element_blank()) +
  scale_y_continuous(labels=scales::percent) +
  ggtitle("First dose dose vaccin admin in % of a total pupolation by DHB")+
  xlab("DHB of residence") +
  ylab("First dose administered, %")+
  theme(legend.position="down")
...
ggplot(df100a, aes(x = New_Var, y =Sum/pop, fill =`DHB of residence`)) +
  geom_bar(stat = "identity", color = "black",position = position_dodge(), width = 0.7)+
  theme(axis.text.x = element_text(face = "bold",angle = 90),
  legend.title = element_blank()) +
  scale_y_continuous(labels=scales::percent) +
...
ggplot(d100, aes(x = New_Var, y =Sum/pop, fill =`name`)) +
  ...
  legend.title = element_blank()) +
  scale_y_continuous(labels=scales::percent) +
  ggtitle("Vaccin admin in % of a total pupolation by DHB")+
  xlab("DHB of residence") +
  ylab("Dose administered, %")
...
df101 = Vaccin_DHB%>%
  group_by(`Name`) %>%
  summarise(`First dose administered` = sum(`First dose administered`))%>%
...
tm1<-tm_shape(nz) +
tm_fill() +
tm_bubbles(size="First dose administered", col="red", size.lim=c(0, 2e7))+
  tm_borders("white", lwd = .5) +
  tm_layout(bg.color = "skyblue", inner.margins = c(0, .02, .02, .02))

tm2 <-tm_shape(nz) +
tm_polygons("First dose administered", title = "First dose administered", palette = "Blues", contrast = 0.7, border.col = "gray30", id = "Name")
...
df101asum<-sum(Vaccin_DHB$`Second dose administered`)

df101a = Vaccin_DHB%>%
  group_by(`Name`) %>%
  summarise(`Second dose administered` = sum(`Second dose administered`))%>%
  mutate(New_Var = paste0(`Name`))
...
tm1 <-tm_shape(nz) +
tm_fill() +
tm_bubbles(size="Second dose administered", col="red", size.lim=c(0, 2e7))+
  tm_borders("white", lwd = .5) +
  tm_layout(bg.color = "skyblue", inner.margins = c(0, .02, .02, .02))
