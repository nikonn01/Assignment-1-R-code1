...
labs(title = paste("Boxplots: Cumulative vaccinations administered, Cumulative vaccinations planned, Cumulative vaccine stock received"))+
...
ggplot(test, aes(x=Period, y = Value, group=Label1, colour=Label1)) + 
  ...
  ggtitle("Cumulative vaccinations administered (tesign dataset) vs predicted values- SVM")+
  xlab("Week ending") +
  ylab("Number of vaccinations administered")+
  labs(fill = "1")+
  theme(legend.position="top")
...
```
Some upper and lover outliers are detected in training data but they are not significant. 
...
ggplot(df4a, aes(x = New_Var, y =Sum/pop, fill =`Ethnic group`)) +
  ...
  ggtitle("First dose vaccinatination administered in % of a total pupolation by Ethnicity for all DHB")+
  xlab("Ethnic Group") +
...
ggplot(df44a, aes(x = New_Var, y =Sum/pop, fill =`Ethnic group`)) +
  ...
  legend.title = element_blank()) +
  scale_y_continuous(labels=scales::percent) +
  ggtitle("Second dose vaccinatination administered in % of a total pupolation by Ethnicity for all DHB")+
  xlab("Ethnic Group") +
