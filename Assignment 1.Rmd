...
## DHB Vaccination Plan: Loading vaccinations data from the Excel file published by the Ministry of Health

```{r read_excel1}

library("readxl")
Vaccin_plan_DHB <- read_excel("covid_vaccinations.xlsx", sheet = "DHB Plan Variance") #column A to be updated; total to be removed; first three rows to be removed

Vaccin_plan_DHB$`DHB`<-factor(Vaccin_plan_DHB$`DHB`)
Vaccin_plan_DHB <- Vaccin_plan_DHB %>% select(-starts_with("Cumulative"))
Vaccin_plan_DHB
```
```{r summarytools1}
library(summarytools)
summarytools::view(summarytools::dfSummary(Vaccin_plan_DHB), method = "render")
```

```{r plot}
vac_variance<-Vaccin_plan_DHB[which(Vaccin_plan_DHB$...2=="Variance"),] #separate variance
vac_variance1 <- as.data.frame(t(vac_variance))


Vaccin_plan_DHB1 <- as.data.frame(t(Vaccin_plan_DHB))
Vaccin_plan_DHB1$Label <- paste0(Vaccin_plan_DHB1$Label, ":00")

library(ggplot2)
ggplot(Vaccin_plan_DHB, aes(x="", y = , group=DHB, colour=DHB)) + 
  geom_line() + 
  geom_point(size = 1)+
  ggtitle("Cumulative vaccinations")+
  xlab("Week ending") +
