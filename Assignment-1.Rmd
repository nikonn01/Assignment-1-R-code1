...
author: "Below analysis has been done by Nadiia Nikoniuk"
date: "11/09/2021"
output:
  html_document:
    toc: yes
...
- COVID-19 vaccination projection for NZ.
...
library("readxl")
Vaccin_DHB <- read_excel("covid_vaccinations_21_09_2021.xlsx", sheet = "DHBofResidence by ethnicity")
...
tm1<-tm_shape(nz) +
tm_fill() +
tm_bubbles(size="First dose administered", col="yellow", size.lim=c(0, 2e7),  border.col = "yellow")+
...
tm2 <-tm_shape(nz) +
tm_fill() +
tm_bubbles(size="Second dose administered", col="green", size.lim=c(0, 2e7), border.col = "green")+
  tm_borders("white", lwd = .5) +
  tm_layout(bg.color = "skyblue", inner.margins = c(0, .02, .02, .02))+
...
ggplot(d, aes(x = New_Var, y =Sum/pop, fill =`name`)) +
  ...
  ggtitle("Vaccin admin in % of a total population by Ethnicity for all DHBs")+
  xlab("Ethnic Group") +
...
ggplot(d, aes(x = New_Var, y =Sum/pop, fill =`name`)) +
  ...
  legend.title = element_blank()) +
  scale_y_continuous(labels=scales::percent, limits=c(0,1)) +
  ggtitle("Vaccin admin in % of a total population by Age group for all DHBs")+
  xlab("Age Group") +
  ylab("Dose administered, %")+
  coord_flip()
...
ggplot(d, aes(x = New_Var, y =Sum/pop, fill =`name`)) +
  ...
  legend.title = element_blank()) +
  scale_y_continuous(labels=scales::percent, limits=c(0,1)) +
  ggtitle("Vaccin admin in % of a total population by Gender for all DHBs")+
  xlab("Gender") +
  ylab("Dose administered, %")+
  coord_flip()
...
#adding latest three date from the Ministry of Helath database as they are missingv - not done for presentation
Observations_CPCOV10 <- rbind(Observations_CPCOV10, c(1, "CPC0V10", "2021/09/05", "Cumulative vaccinations administered",3898365, "Number", "Cumulative Vaccinations", 0))
Observations_CPCOV10 <- rbind(Observations_CPCOV10, c(1, "CPC0V10", "2021/09/12", "Cumulative vaccinations administered",4329697, "Number", "Cumulative Vaccinations", 0))
Observations_CPCOV10 <- rbind(Observations_CPCOV10, c(1, "CPC0V10", "2021/09/19", "Cumulative vaccinations administered",4712992, "Number", "Cumulative Vaccinations", 0))

Observations_CPCOV10 <- rbind(Observations_CPCOV10, c(1, "CPC0V10", "2021/09/05", "Cumulative vaccine stock received",4630860, "Number", "Cumulative Vaccinations", 0))
Observations_CPCOV10 <- rbind(Observations_CPCOV10, c(1, "CPC0V10", "2021/09/12", "Cumulative vaccine stock received",5228730, "Number", "Cumulative Vaccinations", 0))
Observations_CPCOV10 <- rbind(Observations_CPCOV10, c(1, "CPC0V10", "2021/09/19", "Cumulative vaccine stock received",5729490, "Number", "Cumulative Vaccinations", 0))

#Change Value and Periond_Num variables to numeric
Observations_CPCOV10$Value<-as.numeric(Observations_CPCOV10$Value)

#create a dummy numeric variable from the date for Polynomial Regression model
...
```
Observations_CPCOV10 dataframe includes 84 observations and 9 variables. After the data is loaded there are three data types detected: character, numeric and integer. A new dummy variable 'Period_Num' has been created from date for Polynomial Regression as it doesn't handle date format variables. I have updated the 'Period' to date variable, 'Value' and 'Period_Num' to numeric variable,  and 'Label' to factor and its cardinality is 3.  
...
To evaluate the method performance we should split the dataset by testing and training. 
The first 80% of data is assigned to a training and the last 20% is assigned to a testing dataset.
We will train our methods using training dataset and find the optimal hyper parameters for our model. The model with the optimal hyperparameters will be applied to the testing dataset. 
...
The first method I selected is SVM. Lets check how it performs.
```{r svm}
library(e1071)
library(Metrics)
...
There is one residual outlier detected in a testing dataset.
...
tuneResultsvm_ <- tune(svm, Value ~ Period,  data = vaccin_adm, kernel = "polynomial",
              ranges = list(gamma=2^(-1:1), epsilon = seq(0,1,0.1), cost = 2^(seq(0.5,10,.5))))
#best model selection
svm_model <-tuneResultsvm_$best.model
...
-80% population will be vaccinated early November;

-90% population will be vaccinated in the middle of November.

SVM performs well for our dataset.

P.S. In my presentation in the class I used the dataset up to September 2021 and SVM model was showing a strong overfitting. This is why I decided to try another method - Polynomial Regression. I have updated the dataset downloaded from Stats NZ with 3 more dates 5/09/2021, 12/09/2021, 19/09/2021. Data for that dates have been taken from the Ministry of Health website. The main issue is the dataset is very small - we have just 24 observations in our training dataset ans 7 in testing that is why our model is so sensitive to a new data. Below is a vaccination projection using Polynomial Regression. 

### Cumulative vaccination: Vaccination forecast using Polynomial Regression method  
For Polynomial Regression I will find the optimal coefficient for the  training dataset and check how the model performs on  the testing dataset. Polynomial regression works on any size of the dataset and works well on non-liner problems. The main challenge is choosing the right polynomial degree for good bias-variance tradeoff. 
...
employ.data
```
Polynomial Regression model is showing a poor performance on a testing dataset. 
We can plot the predicted and testing values to see how the model behaves. 
...
There are some residual outliers are detected in training dataset. 
...
-70% population will be vaccinated in the middle November;

-80% population will be vaccinated at the end of November;

-90% population will be vaccinated in December

## Conclusion
In this project I have done a Covid-19 vaccination projection using SVM and Polynomial Regression. 

To evaluate the method performance, I have split the dataset by testing and training. The first 80% of data is assigned to a training and the last 20% is assigned to a testing dataset.  

For SVM I have tune hyperparameters using the training dataset. The model with the best hyperparameters has been applied to the testing dataset. In our proposal I selected SVM method because: 

-It is robust to outliers;

-Its implementation is easy with the high accuracy;

-It performs well with the small datasets.

 
For Polynomial Regression I have found the optimal coefficient for the  training dataset and check how the model performs on  the testing dataset. Polynomial regression works on any size of the dataset and works well on non-liner problems. The main challenge is choosing the right polynomial degree for good bias-variance tradeoff. 


The main difference between SVM and Polynomial regression is SVM tries to fit the error in a certain threshold. Polynomial regression is minimizing the error rate.  

For my presentation I used a dataset before Sep 2021 and SVM method was showing a strong overfitting (R-squared for testing dataset) with the data before September 2021 and it potentially could cause  a high variance. This is why I tried a Polynomial Regression which was showing a good performance with R-squared 0.86 for testing dataset.
When I updated the dataframe with September month data the picture has changed cordially. The SVM method performs much better than Polynomial Regression with R-squared 0.25 and many outliers in residuals for the training data. 

As per SVM method we have the following vaccination projection for NZ: 

-70% population can be vaccinated at the end of October 2021;

-80% population can be vaccinated early November 2021;

-90% population can be vaccinated in the middle of November 2021.
